...
def ComputeDistGridCells(pt1, pt2, cells, flagUTM=False):
    ...
    for i in range(len(ptall) - 1):
        ...
        idx1 = id_cells[(cells[:, 0]-1e-6 <= p1[0]) & (p1[0] <= cells[:, 3]+1e-6) & \
                        (cells[:, 1]-1e-6 <= p1[1]) & (p1[1] <= cells[:, 4]+1e-6) & \
                        (cells[:, 2]-1e-6 <= p1[2]) & (p1[2] <= cells[:, 5]+1e-6)]
        #cell indices where the second point belongs 
        idx2 = id_cells[(cells[:, 0]-1e-6 <= p2[0]) & (p2[0] <= cells[:, 3]+1e-6) & \
                        (cells[:, 1]-1e-6 <= p2[1]) & (p2[1] <= cells[:, 4]+1e-6) & \
                        (cells[:, 2]-1e-6 <= p2[2]) & (p2[2] <= cells[:, 5]+1e-6)]
